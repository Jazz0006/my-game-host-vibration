...
import {
  collectWerewolfAfterDeathActions,
...
function gameState(): GameState {
  return {
    config: {
      playerCount: 5,
      roleDeck: ["hunter", "villager", "villager", "seer", "werewolf"],
    },
    phase: "day_result",
    nightNumber: 1,
    dayNumber: 1,
...
type TestRegistry = Readonly<
  Record<Role, WerewolfRoleDefinition<Role, WerewolfInteractionKind>>
>;

describe("Werewolf complex role lifecycle hooks", () => {
  it("expresses the existing hunter death trigger without changing runtime behavior", () => {
    ...
    ).toEqual([{ kind: "hunter_shot", actorPlayerId: "p1" }]);
  ...
  it("supports dynamic alignment without changing the stored role id", () => {
    ...
    const registry: TestRegistry = {
      ...
      seer: {
        ...
        hooks: {
          resolveTeam: ({ game: currentGame }) =>
            currentGame.dayNumber >= 2 ? "wolf" : "village",
        },
      },
    };
  ...
  it("allows a role to override the default winner", () => {
    ...
    const registry: TestRegistry = {
      ...
      seer: {
        ...WEREWOLF_ROLE_REGISTRY.seer,
        hooks: {
          evaluateVictory: ({ rolePlayerId }) =>
            rolePlayerId === "p4" ? "village" : undefined,
        },
      },
    };
  ...
  it("rejects conflicting victory overrides instead of depending on registry order", () => {
    ...
    const registry: TestRegistry = {
      ...
      seer: {
        ...WEREWOLF_ROLE_REGISTRY.seer,
        hooks: { evaluateVictory: () => "village" },
      },
      villager: {
        ...WEREWOLF_ROLE_REGISTRY.villager,
        hooks: {
          evaluateVictory: ({ rolePlayerId }) =>
            rolePlayerId === "p2" ? "wolf" : undefined,
        },
      },
    };
